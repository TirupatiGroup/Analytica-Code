...
const Qtr_Calendar = () => {
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            setUserEname(user.username);  // Assuming the user object contains a name
        }
    }, []);
    ...
    useEffect(() => {
        if (userEname) {
            fetchReportedDates(userEname).then((dates) => {
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-sm font-semibold">
                ...
                {getDaysInMonth(currentDate).map((day) => {
                    ...
                    if (!isCurrentMonthDay) {
                        ...
                        dayClass = 'bg-green-500 text-white shadow-lg cursor-pointer';  // Green background for reported days
                    } else if (isDaySunday) {
